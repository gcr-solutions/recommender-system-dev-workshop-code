...
$AWS_CMD s3 cp ./rs-raw-ec2.yaml s3://${bucket}/rs-dev-workshop-code/latest/ --acl public-read > /tmp/rs-wget.log 2>&1
...
$AWS_CMD s3 cp ./${todayStr}-rs-raw-ec2.yaml s3://${bucket}/rs-dev-workshop-code/${todayStr}/rs-raw-ec2.yaml --acl public-read > /tmp/rs-wget.log 2>&1
...
wget https://github.com/gcr-solutions/recommender-system-dev-workshop-code/archive/refs/heads/main.zip > /tmp/rs-wget.log 2>&1

$AWS_CMD s3 cp main.zip  s3://${bucket}/rs-dev-workshop-code/latest/ --acl public-read > /tmp/rs-wget.log 2>&1
$AWS_CMD s3 cp main.zip  s3://${bucket}/rs-dev-workshop-code/${todayStr}/ --acl public-read > /tmp/rs-wget.log 2>&1
...
if [[ $StableVersion == 'v1' ]]; then
    ...
    $AWS_CMD s3 cp main.zip s3://${bucket}/rs-dev-workshop-code/stable_v1/ --acl public-read > /tmp/rs-wget.log 2>&1
    $AWS_CMD s3 cp ./rs-raw-ec2.yaml s3://${bucket}/rs-dev-workshop-code/stable_v1/ --acl public-read > /tmp/rs-wget.log 2>&1
