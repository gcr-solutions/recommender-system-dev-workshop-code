...
def action_statistics(df_action_input, df_item_input, df_user_input):
    ...
    for row in action_source_total_rows:
        click_count_by_source.append(
            {
                "source": item_type_code_value_dict.get(row['click_source'], "recommend"),
                "source_code": str(row['click_source']),
                "count": row['count']
            }
        )
    ...
    # recommender_click_count = df_action_event_recommender.select('item_id').dropDuplicates(['item_id']).count()
    # statistics_dict['recommender_click_ratio'] = int((recommender_click_count / total_click_item_count) * 100) / 100
