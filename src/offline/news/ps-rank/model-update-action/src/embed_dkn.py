        ...
        with tf.name_scope('embedding'):
            ...
            if params["transform"]:
                ...
                self.entity_embeddings = tf.layers.dense(
                    self.entity_embeddings, units=params["entity_dim"], activation=tf.nn.tanh,
...
def input_fn(filenames='', channel='training', batch_size=32, num_epochs=1, perform_shuffle=False):
        ...
        for i in range(4, 6):
            ...
            def update(sparse, max_click_history, max_title_length):
                ...
                return tf.strings.to_number(
                    tf.slice(string_t, [0, 0], [max_click_history, max_title_length], 'debug_slice_zay'), tf.int32)
    ...
    batch_features = {}
    batch_features['click_words'] = tf.nn.embedding_lookup(word_embeddings,
                                                           tf.reshape(batch_features_index["click_words"],
                                                                      [-1, max_title_length]))
    batch_features['click_entities'] = tf.nn.embedding_lookup(entity_embeddings,
                                                              tf.reshape(batch_features_index["click_entities"],
                                                                         [-1, max_title_length]))
    ...
    if FLAGS.task_type == 'export' or FLAGS.task_type == 'train':
        feature_spec = {
            'click_words': tf.placeholder(
                dtype=tf.float32, shape=[None, model_params["max_title_length"], model_params["word_dim"]],
                name='click_words'),
            'click_entities': tf.placeholder(
                dtype=tf.float32, shape=[None, model_params["max_title_length"], model_params["entity_dim"]],
                name='click_entities'),
