...
file_name_list = ['news_id_news_property_dict.pickle',
                  'news_type_news_ids_dict.pickle', 'filter_config.json']
...
# 加载filter配置
file_to_load = open("info/filter_config.json", "rb")
filter_config = json.load(file_to_load)
print("load filter config {}".format(len(dict_type_id)))
...
def category_diversity_logic(filter_result, stats_result, dict_category_id, filter_config):
    ...
    update_filter_result = dict(
        zip(filter_result_list, filter_result_content_list))
...
for user_id, recall_result in dict_recall_result.items():
    ...
    for recall_id, recall_property in recall_result.items():
        if dict_rank_result.get(str(user_id)) is None:
            logging.warning("Cannot find {} in dict_rank_result".format(user_id))
            continue
        # print("item id {}".format(recall_id))
        # print("dict rank result {}".format(dict_rank_result[str(user_id)]))
        # 构建recall_type
        recall_type = mt_construct(
            run_timing, recall_property[1], recall_property[2])
        # 构建recall_score
