...
emr_action_merged_output_key_prefix = "{}/system/emr/action-preprocessing/output/item_action".format(
    prefix)
emr_action_merged_output_bucket_key_prefix = "s3://{}/{}".format(
    bucket, emr_action_merged_output_key_prefix)
output_action_merged_file_key = "{}/system/item-action-data/item_action.csv".format(prefix)
...
with SparkSession.builder.appName("Spark App - action preprocessing").getOrCreate() as spark:
    ...
    # merge item data
    #
    # ['actionValue', 'userid', 'programId', 'programType', 'actionType', 'timeStamp', 'title', 'category_property']
    df_action_merged = df_action_input.join(df_item, df_action_input["item_id"] == df_item["program_id"], "inner") \
        .select("action_value", "user_id", "item_id", "program_type", "action_type", "timestamp",
                "program_name",
                "category_property")
    df_action_merged.coalesce(1).write.mode("overwrite").option(
        "header", "false").option("sep", "_!_").csv(emr_action_merged_output_bucket_key_prefix)
...
emr_action_merged_output_file_key = list_s3_by_prefix(
    bucket,
    emr_action_merged_output_key_prefix,
    lambda key: key.endswith(".csv"))[0]
print("emr_action_merged_output_file_key:", emr_action_merged_output_file_key)
s3_copy(bucket, emr_action_merged_output_file_key, output_action_merged_file_key)
print("output_action_merged_file_key:", output_action_merged_file_key)
