...
def gen_pickle_files(item_s3_path, out_s3_path):
    logging.info(f"gen_pick_files(), "
                 f"\nitem_s3_path={item_s3_path}, "
                 f"\nout_s3_path={out_s3_path}")

    df = prepare_df(item_s3_path)
...
if __name__ == '__main__':
    ...
    item_s3_path = f"s3://{bucket}/{prefix}/system/item-data/item.csv"
    out_s3_path = f"s3://{bucket}/{prefix}/feature/content/inverted-list"
    gen_pickle_files(item_s3_path, out_s3_path)
