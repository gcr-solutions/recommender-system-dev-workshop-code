...
#         dataset = dataset.shard(hvd.size(), hvd.rank())
        ...
        #shuffle的buffer size最好设置至少为训练集的大小，这里设置为1M只是为了测试他对训练速度的影响。
        if perform_shuffle:
            dataset = dataset.shuffle(buffer_size=1024*1024)
        
        dataset = dataset.map(decode_libsvm, num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
#     hvd.init()
    ...
    print("%s/tr*libsvm" % FLAGS.data_dir)
...
#     if hvd.rank() == 0:
#         DeepFM = tf.estimator.Estimator(model_fn=model_fn, model_dir=FLAGS.checkpoinPath, params=model_params, config=tf.estimator.RunConfig().replace(session_config=config))
#     else:
...
#     eval_channel=channel_names[0]
    ...
    perform_shuffle=FLAGS.perform_shuffle
    ...
    if FLAGS.task_type == 'train':
        ...
        if FLAGS.pipe_mode == 0: #file mode
            for _ in range(FLAGS.num_epochs):
                DeepFM.train(input_fn=lambda: input_fn(tr_files, num_epochs=1, batch_size=FLAGS.batch_size, perform_shuffle=perform_shuffle))
#                 DeepFM.train(input_fn=lambda: input_fn(tr_files, num_epochs=1, batch_size=FLAGS.batch_size, perform_shuffle=perform_shuffle), hooks=[bcast_hook])
