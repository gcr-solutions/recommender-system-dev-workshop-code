...
class Retrieve(service_pb2_grpc.RetrieveServicer):
    ...
    def get_recommend_result(self, user_id, recommend_type):
        ...
        logging.info("get_recommend_result return recommend_list size: {}".format(len(recommend_list)))
        return recommend_list
    ...
    def get_ps_recommend_result(self, user_id, recommend_type):
        ...
        ps_recommend_list = []
        if recommend_type == 'recommend':
            logging.info('personalize recommend news list to user')
            ...
            if not bool(self.news_type_news_ids_dict):
                ps_recommend_list
            news_id_list = self.news_type_news_ids_dict[recommend_type]
            ps_recommend_list = self.generate_news_list_by_type(news_id_list)
    ...
    # def generate_ps_cold_start_data(self, user_id):
    #     logging.info('start personalize cold start algorithm')
    #     coldstart_item_list = {}
    #     new_ps_record = []
    #     self.personalize_runtime = boto3.client('personalize-runtime', MANDATORY_ENV_VARS['AWS_REGION'])
    #     # trigger personalize api
    ...
    #     result_list = get_recommendations_response['itemList']
    #     for item in result_list:
    #         coldstart_item_list[item['itemId']] = [item['itemId'], "recommend", item['score'],
    #                                                "personalize_cold_start|{}".format(str(item['score']))]
    #     i = 0
    #     while len(coldstart_item_list) < int(MANDATORY_ENV_VARS['COLDSTART_NEWS_COUNT']):
    #         index = i % len(self.lCfgCompleteType)
    #         temp_item_id = self.sample_by_type(self.lCfgCompleteType[index])
    ...
    #         logging.info('get sample item id {} by type {}'.format(temp_item_id, self.lCfgCompleteType[index]))
    #         if temp_item_id not in coldstart_item_list:
    #             desp = [str(temp_item_id), "recommend", 0, str(self.lCfgCompleteType[index])]
    #             coldstart_item_list[str(temp_item_id)] = desp
    #         i = i + 1
    #
    #     new_ps_record.append({
    #         calendar.timegm(time.gmtime()): coldstart_item_list
    #     })
    #
    #     logging.info('personalize coldstart filter record {}'.format(new_ps_record))
    #
    #     if rCache.load_data_into_hash(user_id_personalize_dict, user_id, json.dumps(new_ps_record).encode('utf-8')):
    #         logging.info('Save personalize data into Redis with key : %s ', user_id)
    #     return new_ps_record
    ...
    def generate_new_recommend_data(self, user_id, filtered_data):
        logging.info('generate_new_recommend_data start, filtered_data:{}'.format(filtered_data))
        ...
        if self.is_cold_start_data(filtered_data):
            ...
            new_recommend_list = self.get_present_recommend_news_list(filtered_data)
        else:
    ...
    def generate_new_ps_recommend_data(self, user_id, personalize_data):
        logging.info('generate_new_ps_recommend_data start, personalize_data:{}'.format(personalize_data))

        new_ps_recommend_list = []
        # iterate filter data and get recommend
        if self.is_cold_start_data(personalize_data):
            logging.info('is cold start data')
            # new_recommend_list: [{'id':'news_id1', 'tag': 'recommend'},{'id':'news_id2', 'tag': 'recommend'}]
            # present_recommend_news_id_list: ['news_id1','news_id2']
            new_ps_recommend_list = self.get_present_recommend_news_list(personalize_data)
        else:
            hot_topic_count = hot_topic_count_array[random.randint(0, len(hot_topic_count_array) - 1)]
            new_ps_recommend_list = self.get_present_recommend_news_list(personalize_data)
            logging.info('need hot topic news')
            ...
            # comment for dev workshop
            hot_topic_news_list = self.get_hot_topic_news_list(user_id, hot_topic_count)
            new_ps_recommend_list = hot_topic_news_list + new_ps_recommend_list

        return new_ps_recommend_list
    ...
    def get_present_recommend_news_list(self, filtered_data):
    ...
    def get_hot_topic_type(self, user_id):
        ...
        if len(user_portrait_result) == 0:
            # cannot find user portrait
            return ''
