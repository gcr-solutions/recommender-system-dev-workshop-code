...
class ServiceImpl:
    ...
    def recall_by_portrait(self, user_portrait, recall_wrap, recall_items, multiple_shot_record):
        ...
        user_ub_embedding = np.array(user_portrait['ub_embeddding'])
        user_ub_embedding = user_ub_embedding.astype('float32')
        # logging.info("!!!!!!current embedding {} with type {} dtype {}".format(user_ub_embedding, type(user_ub_embedding), user_ub_embedding.dtype))
        # logging.info("!!!!!!current method {} with type {}".format(topn_wrap['portrait_ub'], type(topn_wrap['portrait_ub'])))
        ub_faiss_index = recall_wrap['ub_index']
        ub_idx_mapping = recall_wrap['ub_idx_mapping']
        D, I = ub_faiss_index.search(np.ascontiguousarray(user_ub_embedding), int(topn_wrap['portrait_ub']))
