...
class Rank(service_pb2_grpc.RankServicer):
    ...
    def __init__(self):
        ...
        pickle_file_list = [MANDATORY_ENV_VARS['NEWS_ID_WORD_IDS'], MANDATORY_ENV_VARS['NEWS_ID_ENTITY_IDS']]
        ...
        for file_name in file_list:
            ...
            if MANDATORY_ENV_VARS['NEWS_ID_ENTITY_IDS'] in pickle_path:
                if os.path.isfile(pickle_path):
                    ...
                    logging.info('reload news_id_entity_ids_dict, file is empty')                   
            if MANDATORY_ENV_VARS['NEWS_ID_WORD_IDS'] in pickle_path:
                if os.path.isfile(pickle_path):
                    ...
                    logging.info('reload news_id_word_ids_dict, file is empty')                
    ...
    def generate_rank_result(self, recall_result, news_id_entity_ids_dict, news_id_word_ids_dict, user_clicks_set):
        ...
        for recall_item in recall_result:
            logging.info('recall_item news id {}'.format(recall_item[0]))
            news_words_index.append(news_id_word_ids_dict[recall_item[0]])
            news_entity_index.append(news_id_entity_ids_dict[recall_item[0]])

            click_length = len(user_clicks_set)
            count = 0
            while click_length > 0 and count < 8:
                click_news_id = user_clicks_set[click_length - 1]
                logging.info('clicked_item_id {}'.format(click_news_id))
                logging.info('news_id_word_ids_dict {}'.format(news_id_word_ids_dict[click_news_id]))
                logging.info('news_id_entity_ids_dict {}'.format(news_id_entity_ids_dict[click_news_id]))
                click_words_index.append(news_id_word_ids_dict[click_news_id])
                click_entity_index.append(news_id_entity_ids_dict[click_news_id])
